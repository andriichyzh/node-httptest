...
HttpTest.prototype.setParam = function(param, value) {
    ...
    var source = this.option.current.params || this.option.common.params,
        data = {};
    ...
    this.option.current.params = _.extend(source, data);
...
HttpTest.prototype.expectStatus = function(status) {
    ...
    if (this.expect.current.status) {
        this.expect.current.status = status;
    } else {
        this.option.common.headers = _.extend(this.option.common.headers, data);
    }
    
    this.expect.status = status;
    return this;
};
...
HttpTest.prototype.setQuery = HttpTest.prototype.setParam;
